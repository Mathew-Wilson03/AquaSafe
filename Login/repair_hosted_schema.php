<?php
/**
 * AquaSafe Hosted Schema Repair Tool
 * 
 * This script fixes column name inconsistencies between different 
 * versions of the IoT and Dashboard code.
 */
require_once 'config.php';

echo "<h2>AquaSafe Schema Repair Tool</h2>";
...
// 1. Fix flood_data: level (from water_level)
checkAndFix($link, 'flood_data', 'water_level', 'level', "DECIMAL(10,2) NOT NULL");

// 2. Fix flood_data: created_at (from timestamp)
checkAndFix($link, 'flood_data', 'timestamp', 'created_at', "TIMESTAMP DEFAULT CURRENT_TIMESTAMP");

// 3. Fix sensor_status: water_level (ensure it exists)
// Note: sensor_status uses water_level in the current code, but we check anyway
...
echo "<hr><p><b>Repair process complete.</b> Please check your dashboard now.</p>";
?>
